...
- **V4 data format**: 1 row per grade per fiscal year (6 grades × 1 row = 6 rows/year), with 12 monthly columns. Each grade independently chooses `annual` mode (single total) or `monthly` mode (per-month targets)
...
- **Enabled weekdays**: stored in `PropertiesService` as `MODULE_WEEKDAYS_ENABLED` (comma-separated day numbers, e.g. `1,3,5`). Read by `getEnabledWeekdays()` in `moduleHoursControl.js`, serialized by `serializeWeekdays()`.
- **Weekday priority**: `MODULE_WEEKDAY_PRIORITY` in `moduleHoursConstants.js` defines allocation order (Mon=0, Wed=1, Fri=2). `weekdayPriority()` in `moduleHoursPlanning.js` loads with per-execution cache (`activeWeekdayPriorityMap_`). Call `resetWeekdayPriorityCache_()` after saving new priority values.
