 ...
 * 対象日（「年度更新作業」シートのC11セルの日曜日の翌日＝4月1日）から366行分の  
...
function importAnnualEvents() {
  ...
  // 3. アクティブなスプレッドシート（ポータルマスター）の「年度更新作業」シートのC11セルから、
  ...
  var sundayDate = updateSheet.getRange("C11").getValue();
  if (!(sundayDate instanceof Date)) {
    sundayDate = new Date(sundayDate);
    if (isNaN(sundayDate.getTime())) {
      ui.alert("年度更新作業シートのC11セルに有効な日付が設定されていません。");
      return;
    }
  }
  ...
  var sourceDispValues = sourceSheet.getRange(1, 1, 10, 1).getDisplayValues();
  Logger.log("sourceSheet A列上位10行 (getDisplayValues): " + JSON.stringify(sourceDispValues));
  
  // 5. コピー元シートのA列上位10行の【生データ】を取得し、対象日がある行番号を決定するため、  
  //    値が Date オブジェクトでない場合は、"X月Y日" 形式であれば対象年を補完して "yyyy/MM/dd" 形式に変換する
  var sourceValues = sourceSheet.getRange(1, 1, 10, 1).getValues();
  var sourceLogConverted = [];
  var sourceStartRow = null;
  for (var i = 0; i < sourceValues.length; i++) {
    var cellValue = sourceValues[i][0];
    var cellString = convertCellValue(cellValue, targetDate.getFullYear());
    sourceLogConverted.push(cellString);
    if (!sourceStartRow && cellString === targetDisplayString) {
      sourceStartRow = i + 1; // 行番号は1から始まる
    }
  }
  Logger.log("sourceSheet A列上位10行 (converted): " + JSON.stringify(sourceLogConverted));
  if (!sourceStartRow) {
    ui.alert("コピー元シートのA列（上位10行）に対象の日付 (" + targetDisplayString + ") が見つかりませんでした。");
    return;
  }
  Logger.log("コピー元対象開始行: " + sourceStartRow);
  
  // 6. 貼り付け先シート「マスター」のA列上位10行の【表示値】を取得し、対象日がある行番号を決定する
  var masterSheet = activeSpreadsheet.getSheetByName("マスター");
  if (!masterSheet) {
    ui.alert("マスターが見つかりません。");
    return;
  }
  var masterValues = masterSheet.getRange(1, 1, 10, 1).getValues();
  var masterLogConverted = [];
  var destStartRow = null;
  for (var j = 0; j < masterValues.length; j++) {
    var cellValue = masterValues[j][0];
    var cellString = convertCellValue(cellValue, targetDate.getFullYear());
    masterLogConverted.push(cellString);
    if (!destStartRow && cellString === targetDisplayString) {
      destStartRow = j + 1;
    }
  }
